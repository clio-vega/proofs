...
write $\ht(\theta)=(\text{number of rows of }\theta)-1$ for its leg
...
\[
\ht(\mu/\lambda)=\#\{s: 1\le s\le e-1,\ j+s\in S\},
\]
...
\sum_\mu t^{\ht(\mu/\lambda)}\mu$ over $e$-ribbon additions.
...
$\ht=|\varepsilon|$, hence the entire $t$-line is an orientation
