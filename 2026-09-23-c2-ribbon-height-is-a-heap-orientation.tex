...
24 September 2026 (repository log from \texttt{0a01bb9} onward), and they are
listed by kind because
``nothing has changed'' is a claim about every other line and survives nothing:
(i) this block was added;
(ii) \texttt{\textbackslash newcommand\{\textbackslash ht\}} \emph{silently failed} --- \texttt{\textbackslash ht} is a \TeX{} primitive,
so all six uses were parsed as box-dimension lookups and rendered as nothing, in a note
whose subject is ribbon height; it is now \texttt{\textbackslash DeclareMathOperator\{\textbackslash hgt\}\{ht\}};
(iii) the \texttt{]} inside the optional argument of Theorem~\ref{thm:A} closed it early,
