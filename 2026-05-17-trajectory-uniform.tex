...
\subsection{The $\sigma$-invariant}\label{sec:sigma}

Denote $\sigma(T):=\{$entries of $T$ in column $2\}$ for an SYT $T$
of $\lambda$ or $\mu_1$.  Choose the convention $P_{k+1}^+:=$ the
surviving element of $\{P_k^+,P_k^-\}$ after Lemma~\ref{lem:kill-expand}.

\begin{proposition}[$\sigma$-invariant]\label{prop:sigma}
For every $k\in\{0,1,\ldots,a-1\}$,
\begin{equation}\label{eq:sigma-inv}
   \sigma(P_k^+)\cap\{n-2a+1,\ldots,n-2-k\}
   \;=\;
   \sigma(T')\cap\{n-2a+1,\ldots,n-2-k\}.
\end{equation}
\end{proposition}

The range $\{n-2a+1,\ldots,n-2-k\}$ omits the ``active'' entries
$e_2^{(k)}=n-1-k$ and $e_3^{(k)}=n-k$ (and all higher), capturing
only the entries on which $\sigma(P_k^+)$ is ``frozen'' from
$\sigma(T')$.

\begin{proof}
Induction on $k$.

\emph{Base $k=0$:}  $P_0^+=T_A(T')$, $\sigma(T_A)=\sigma(T')\cup\{n-1\}$.
Since $\sigma(T')\subseteq\{n-2a+1,\ldots,n-2\}$ and $n-1$ is outside
the range $\{n-2a+1,\ldots,n-2\}$,
$\sigma(T_A)\cap\{n-2a+1,\ldots,n-2\}=\sigma(T')=
\sigma(T')\cap\{n-2a+1,\ldots,n-2\}$.

\emph{Inductive step $k\to k+1$.}  Assume \eqref{eq:sigma-inv} at $k$.
\[
   \sigma(P_k^+)\cap\{n-2a+1,\ldots,n-2-k\}
   =\sigma(T')\cap\{n-2a+1,\ldots,n-2-k\}.
\]
By the convention, $P_{k+1}^+$ is the survivor of
$\{P_k^+,P_k^-\}$ at step $k+1$.

\emph{Key observation.}  $P_k^-=s_{e_2}P_k^+$ for $e_2=n-1-k$, so
$\sigma(P_k^-)$ differs from $\sigma(P_k^+)$ only in entries
$\{e_2,e_3\}=\{n-1-k,n-k\}$, both of which lie strictly
\emph{above} $n-2-k$.  Hence
\[
   \sigma(P_k^-)\cap\{n-2a+1,\ldots,n-2-k\}
   =\sigma(P_k^+)\cap\{n-2a+1,\ldots,n-2-k\}.
\]
So \emph{both} elements of the pair carry the same lower-range
$\sigma$.  The survivor $P_{k+1}^+$, whether equal to $P_k^+$ or
$P_k^-$, satisfies
\[
   \sigma(P_{k+1}^+)\cap\{n-2a+1,\ldots,n-2-k\}
   =\sigma(T')\cap\{n-2a+1,\ldots,n-2-k\}.
\]
Restricting to the smaller range $\{n-2a+1,\ldots,n-3-k\}$ (dropping
$n-2-k$, the new $e_2^{(k+1)}=n-2-k$):
\[
   \sigma(P_{k+1}^+)\cap\{n-2a+1,\ldots,n-3-k\}
   =\sigma(T')\cap\{n-2a+1,\ldots,n-3-k\}.
\]
This is the invariant at $k+1$.
\end{proof}

\begin{corollary}[Encoding bit identification]\label{cor:encoding}
For every $k\in\{0,1,\ldots,a-2\}$,
\[
   e_1=n-2-k\in\sigma(P_k^+)
   \;\Longleftrightarrow\;
   \tau_{k+1}(T')=R,
\]
where $\tau(T')\in\{L,R\}^{a-1}$ is the $S_{a-1}$-encoding of $T'$.
Equivalently, Sub-case (b) of Lemma~\ref{lem:kill-expand} applies at
step $k+1$ iff $\tau_{k+1}=R$.
\end{corollary}

\begin{proof}
By Proposition~\ref{prop:sigma}, $e_1\in\sigma(P_k^+)$ iff
$e_1\in\sigma(T')$ (since $e_1=n-2-k$ lies in the lower range
$\{n-2a+1,\ldots,n-2-k\}$, where $\sigma(P_k^+)$ matches $\sigma(T')$).

It remains to identify $e_1\in\sigma(T')$ with $\tau_{k+1}=R$.
Set $\sigma':=\sigma(T')-(n-2a+1)\in S_{a-1}\subseteq[2a-2]$;
then $e_1\in\sigma(T')$ iff $2a-3-k\in\sigma'$.  We claim:
\begin{equation}\label{eq:tau-bit-rule}
   \tau_{k+1}(\sigma')\;=\;R\;\Longleftrightarrow\;(2a-3-k)\in\sigma'
   \qquad(k=0,\ldots,a-2).
\end{equation}

\emph{Proof of \eqref{eq:tau-bit-rule}.}  Induction on $k$.
The encoding $\varepsilon:S_{a-1}\to\{L,R\}^{a-1}$ is defined by:
$\tau_1(\sigma')=L$ if $0\in\sigma'$, $R$ if $2a-3\in\sigma'$;
strip the chosen ``boundary'' element ($0$ or $2a-3$); shift the
remainder down by $1$; recurse to define $\tau_2,\ldots,\tau_{a-1}$
on the resulting element of $S_{a-2}\subseteq[2a-4]$.

\emph{Base $k=0$:}  $\tau_1=R$ iff $2a-3\in\sigma'$, by definition.
$\checkmark$

\emph{Inductive step $k\to k+1$ for $k\le a-3$:}  Let $\sigma''$ be
the element of $S_{a-2}$ obtained from $\sigma'$ by one strip-shift.
By the inductive hypothesis on $\sigma''\in S_{a-2}\subseteq[2a-4]$:
\[
   \tau_{k+2}(\sigma')=\tau_{k+1}(\sigma'')=R
   \iff (2(a-1)-3-k)\in\sigma''
   \iff (2a-5-k)\in\sigma''.
\]
Now $\sigma''=(\sigma'\setminus\{0\text{ or }2a-3\})-1$ (strip then
shift), so $x\in\sigma''$ iff $x+1\in\sigma'\setminus\{0,2a-3\}$.
For $x=2a-5-k$ and $k\le a-3$, $x+1=2a-4-k$; this is neither $0$
(requires $k=2a-4$, contradicting $k\le a-3$) nor $2a-3$ (requires
$k=-1$).  So
\[
   (2a-5-k)\in\sigma''\iff(2a-4-k)\in\sigma'.
\]
Hence $\tau_{k+2}(\sigma')=R\iff(2a-4-k)\in\sigma'$, which matches
\eqref{eq:tau-bit-rule} at index $k+1$ since $2a-3-(k+1)=2a-4-k$.
\end{proof}

\begin{remark}
\eqref{eq:tau-bit-rule} says: as the encoding strips off bits, each
new bit ``tests'' a different element of $\sigma'$.  Specifically,
the $k+1$-th bit tests element $2a-3-k$ (counting from the maximum
of $\sigma'$'s universe downward).  This pattern is forced by the
recursive shape of $S_{a-1}$.
\end{remark}

\subsection{Toward the $S_a^{(n)}$ support identification}\label{sec:remaining}

\paragraph{Final pair structure.}  At $k=a-1$, the pair
$\{P_{a-1}^+,P_{a-1}^-=s_{n-a}P_{a-1}^+\}$ is a non-degenerate
$T_{n-a}$-2-block.  By Proposition~\ref{prop:sigma},
$\sigma(P_{a-1}^+)\cap\{n-2a+1,\ldots,n-a-1\}=
\sigma(T')\cap\{n-2a+1,\ldots,n-a-1\}$.

The pair $\{P_{a-1}^+,P_{a-1}^-\}$ differs only in the membership of
$\{n-a,n-a+1\}$ in $\sigma$: one has $n-a$ in col-$2$ (and
$n-a+1$ in col-$1$), the other vice-versa.

\paragraph{Encoding identification (sketch).}  By
Corollary~\ref{cor:encoding}, the trajectory's kill/expand history is
determined by $\tau$.  Tracking $\sigma(P_k^+)$ through the
trajectory (using the explicit ``$\sigma(s_{e_2}P)=\sigma(P)
\triangle\{e_2,e_3\}$'' rule and the kill identification of
Lemma~\ref{lem:kill-expand}), one shows by induction:

\begin{conjecture}[Final pair $=\{T_{\tau L},T_{\tau R}\}$]\label{conj:final-pair}
For each $T'\in S_{a-1}^{(n-2)}$ with encoding $\tau=\varepsilon(T')$,
the final pair $\{P_{a-1}^+(T'),P_{a-1}^-(T')\}$ equals
$\{T_{\tau L},T_{\tau R}\}\subseteq S_a^{(n)}$ with first $(a-1)$
encoding bits matching $\tau$ and last bit varying $\in\{L,R\}$.
\end{conjecture}

This is May-16 Lemma~4.3 generalized.  Combined with
Proposition~\ref{prop:sigma} and Corollary~\ref{cor:encoding}, the
remaining residue is the explicit identification of $\sigma$ at the
``upper end'' (entries $\{n-a,\ldots,n\}$), which we have not
formalized here.

\paragraph{Verification at $a=2$.}  For $T'\in S_1^{(n-2)}$:
\begin{itemize}[leftmargin=2em,topsep=0.3em,itemsep=0.2em]
\item $T'_L$, $\sigma(T')=\{n-3\}$.  $T_A=$ extend by $n-1$ at $(2,2)$,
  $n$ at $(n-2,1)$.  Step~$0$ applies $T_{n-2}$, $e_1=n-2$.
  $e_1\notin\sigma(T_A)$: Sub-case II.a.  Pair after step~$0$:
  $\{T_A,s_{n-2}T_A\}$ with $\sigma$ values
  $\{\sigma(T')\cup\{n-1\},\sigma(T')\cup\{n-2\}\}=\{\{n-3,n-1\},\{n-3,n-2\}\}$.
  Shifted to $\{0,1,2,3\}$ (subtract $n-3$):
  $\{\{0,2\},\{0,1\}\}$, encodings $\{LR,LL\}=\{\tau L,\tau R\}$ with
  $\tau=L$.  $\checkmark$
\item $T'_R$, $\sigma(T')=\{n-2\}$.  Analogous: pair $\sigma$ values
  $\{\{n-2,n\},\{n-1,n\}\}$, shifted to $\{1,3\},\{2,3\}$, encodings
  $RL,RR$.  $\checkmark$
\end{itemize}

Combined with Conjecture~\ref{conj:final-pair} (verified at $a=2,3,4$
by May-13/May-15-a3/May-16, and shown to follow from
Proposition~\ref{prop:sigma} at $a=2$ above):

\begin{corollary}[Support identification, conditional]\label{cor:support}
Assume Conjecture~\ref{conj:final-pair}.  Then for every $a\ge 2$,
$n\ge 3a$, $B_{n-a+1}^{(\lambda)}V_\lambda$ is exactly
$1$-dimensional with seminormal support $S_a^{(n)}$ ($|S_a^{(n)}|=2^a$),
all coefficients nonzero in $\mathbb{Q}(q)$.  Different
$T'\in S_{a-1}^{(n-2)}$ give pairs with disjoint encoding prefixes,
hence disjoint supports.
\end{corollary}

The residue between the structural lemma and the full support claim
is just the upper-end tracking, which is now a clean book-keeping
problem rather than a global combinatorial mystery.
