...
\end{itemize}
\emph{Which engine warrants what.} E1 is Proposition~\ref{prop:reflect} compiled into code and
E1b is Theorem~\ref{thm:struct} compiled into code; both are \emph{claim-derived}, so comparing
them to each other is an implement-and-compare consistency check on the derivations, not
independent evidence. \textbf{E2 is the only engine independent of everything proved here}, and
every claim below is checked against E2. The warrant for Theorem~\ref{thm:struct} is the
derivation in \S\ref{sec:struct}; the warrant for Proposition~\ref{prop:reflect} is
\S\ref{sec:reflect}, whose \emph{prediction} --- the explicit row \eqref{eq:lam} --- is then
looked up in E2, which is a genuinely independent test.
