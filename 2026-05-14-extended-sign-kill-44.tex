...
For $\lambda = (4, 4, 1^q)$ at $q \in \{6, 7\}$ (and conjecturally
for all $q \ge 6$), $\supp(B)$ contains an SYT $T$ with $p(T)
= n - 12 = q - 4$ exactly; consequently
$B \not\subseteq E_{q-4}^-|_{V_\lambda}$.
\end{theorem}

\begin{proof}
Existence at $q \in \{6, 7\}$ is verified computationally
...
\item Sharpness witness at $q = 7$ ($n = 15$, $\dim V_\lambda = 13\,650$):
   an SYT $T \in \supp(B)$ with $p(T) = q - 4 = 3$ exactly:
   $T(1, \cdot) = (1, 4, 5, 6),\ T(2, \cdot) = (2, 7, 8, 9),\
   T(3, 1) = 3,\ T(4..9, 1) = (10, 11, 12, 13, 14, 15)$.
\item Both $|\supp(\Om \cdot e_i)|$ patterns at $q \in \{6, 7\}$
   stabilise at $2050$ for random probe columns, suggesting the support
   structure of $B$ stabilises in the full regime.
\end{itemize}
\end{theorem}
...
\item Theorem~\ref{thm:sharp}: sharpness at $j = q - 4$ for
   $q \in \{6, 7\}$ (computational witness).
\end{itemize}
