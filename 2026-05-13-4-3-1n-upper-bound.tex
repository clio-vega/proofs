...
We prove the matching statement at the $j$-formula level: for every
$q \ge 3$,
\[
...
\]
Both statements are \emph{fully structural} at $q \ge 5$. At
$q \in \{3, 4\}$, the four-branch reduction itself is structural (the
two zero pieces $\mu_C$-leakage and same-row-dies are structural at
$q \ge 3$), but the two non-zero $\Phi$-summand inputs come from
families $(3, 3, 1^{q-1})$ and $(4, 2, 1^{q-1})$ at sub-shape sizes
$8$ or $9$, where the relevant $j$-formulas and dim formulas have only
computational verification (the structural ranges of SRD-331 and
SRD-421 cut in at sub-shape size $\ge 10$). Combined with the
computational lower bound $\dim B \ge 5$, this gives $\dim B = 5$
on every $q \in \{3, 4, 5, \ldots\}$ in the ranges tested.
...
(May-13 same-row-dies-$(4,2,1^*)$ + May-12-Eve3, structural at
$|\mu_B| \ge 10$, i.e.\ $q - 1 \ge 4$).
...
\begin{theorem}[$\mu_B$: dim formula and $j$-formula on $(4, 2, 1^*)$, May-12-Eve3 + SRD-421]\label{thm:muB}
For $\mu_B = (4, 2, 1^{q-1})$:
\begin{enumerate}[leftmargin=2em,topsep=0.2em,itemsep=0.1em]
\item $\dim B^{(\mu_B)}_{j_0(\mu_B)} V_{\mu_B} = 3$ \emph{structurally}
at $|\mu_B| \ge 10$ (i.e.\ $q - 1 \ge 4$, equivalently $q \ge 5$); for
$q - 1 \in \{2, 3\}$, $\dim = 3$ is established computationally
(mod $P = 100\,003$, $Q = 11$).
\item $B^{(\mu_B)}_{j_0(\mu_B)} V_{\mu_B} \subseteq E_1^-\!\mid_{V_{\mu_B}}$
\emph{structurally} at $|\mu_B| \ge 10$ (i.e.\ $q \ge 5$); for smaller
$|\mu_B|$ it is established computationally.
\end{enumerate}
\end{theorem}

\begin{remark}\label{rem:muB-structural-range}
The structural range $|\mu_B| \ge 10$ inside SRD-421 comes from
sub-shape dependencies: the upper bound and $E_1^-$ containment on
$(4, 2, 1^*)$ rely on the $j$-formula on $(3, 2, 1^*)$ (May-12 today,
structural at size $\ge 8$). Same-row dies for $(4, 2, 1^*)$ alone
is structural at all $q - 1 \ge 2$, but does not by itself give the
$E_1^-$ containment on $\mu_B$.
\end{remark}
...
\]
This is structural at $q \ge 5$; at $q \in \{3, 4\}$ it relies on the
computational $E_1^-$ containment for $\mu_A = (3, 3, 1^{q-1})$
(automatic, see Theorem~\ref{thm:muA}(1) for $q \ge 3$) and for
$\mu_B = (4, 2, 1^{q-1})$ at $|\mu_B| \in \{8, 9\}$
(computational input from Theorem~\ref{thm:muB}(2)).
\end{theorem}
...
\]
Note: Lemma~\ref{lem:muC-zero} (the $\mu_C$-leakage) and
Lemma~\ref{lem:S-zero} (same-row dies) are fully structural at every
$q \ge 3$; the conditional pieces are the two $\Phi_X$-summands above.

For the $E_1^-$ containment: by Theorems~\ref{thm:muA}(1)
(structural at $q \ge 3$) and~\ref{thm:muB}(2)
(structural at $q \ge 5$; computational at $q \in \{3, 4\}$),
both inner images lie in $E_1^-$ of the respective $V_{\mu_X}$. By
...
with $q - 1 \in \{2, 3\}$ is computational
(Theorem~\ref{thm:muA}(2)), and $\dim B^{(\mu_B)} = 3$ at
$\mu_B = (4, 2, 1^{q-1})$ with $q - 1 \in \{2, 3\}$ (i.e.\
$|\mu_B| \in \{8, 9\}$) is also computational
(Theorem~\ref{thm:muB}(1)).\qed
\end{proof}
...
Theorem~\ref{thm:main-upper}'s upper bound at $q \in \{3, 4\}$ relies
on the computational dim $= 2$ for $\mu_A = (3, 3, 1^{q-1})$ at
$q - 1 \in \{2, 3\}$ \emph{and} on the computational dim $= 3$ for
$\mu_B = (4, 2, 1^{q-1})$ at $|\mu_B| \in \{8, 9\}$. Structural
proofs of either dim, at sub-shape sizes $8$--$9$, would propagate
this paper's upper bound to fully structural at the corresponding
$q$ values. The smaller sub-shapes are also closer to the rank-zero
boundary, so a separate small-case treatment may be required.
...
For $\lambda = (4, 3, 1^q)$ at every $q \ge 3$:
\begin{enumerate}[leftmargin=2em,topsep=0.2em,itemsep=0.1em]
\item The four-branch reduction structurally identifies
$B^{(\lambda)}_{j_0} V_\lambda$ with $(\text{outer factors})
\cdot \Phi_A(B^{(\mu_A)}) + (\text{outer factors})
\cdot \Phi_B(B^{(\mu_B)})$ (Theorem~\ref{thm:reduction} +
Lemmas~\ref{lem:muC-zero},~\ref{lem:S-zero}). This reduction
itself is fully structural at $q \ge 3$: the two zero pieces
($\mu_C$-leakage and same-row-dies) use only May-12-today,
SRD-421-Theorem-14, and the Shift Lemma all-hooks $j$-formula.
...
Structural at $q \ge 5$; conditional on the computational
$\mu_B$-input at $q \in \{3, 4\}$.
\item The upper bound $\dim B^{(\lambda)}_{j_0} V_\lambda \le 5$
(Theorem~\ref{thm:main-upper}), structural at $q \ge 5$ and
conditional on computational dim inputs from
$\mu_A, \mu_B$ at $q \in \{3, 4\}$.
\end{enumerate}
