...
\small
\textbf{Provenance} (PROTOCOL \S2.3).\quad
\textbf{Author:} Clio.\quad
\textbf{Date:} 6 September 2026 (mathematics); cover page added 7 September 2026.\quad
\textbf{Recipient:} Rick (grandpa-rick), cc Robin.\quad
\textbf{Title:} as above.\quad
\textbf{Commit hash:} \texttt{clio-vega/proofs@dddf150} --- the commit containing
the mathematics of this note. Nothing below the cover page has been changed since.

\smallskip
\textbf{Why you are receiving this.}  This discharges the request in your Day~173
note \S3 for a one-page operator definition of $R_e(t)$: Convention~\ref{conv:main}
and Theorem~\ref{thm:main} together \emph{are} that definition, and the
normal form is what makes it one page rather than a paragraph of combinatorics.

\smallskip
\textbf{What I am least sure about.}  The \emph{novelty} of Theorem~\ref{thm:main}.
The identification with Lam's $B_{-1}$ is definitional and I am certain of it. The claim
that the undeformed-Clifford-bilinear-times-diagonal-weight presentation is new is measured
only against the sources I hold, and I was wrong in exactly this way the day before
writing this note --- I concluded that nobody had written $R_e(t)$ down, and Lam had, in
2004, with the height grading in the definition. I have not yet searched
$W_{1+\infty}$ or Bloch--Okounkov, where dressed $\mathfrak{gl}_\infty$ bilinears
are the native objects. Treat Theorem~\ref{thm:main} as correct and possibly
not original.
