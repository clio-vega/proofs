...
\begin{theorem}[Verified at $n=9,10,11$]
Theorem~\ref{thm:main} and the intermediate
Propositions~\ref{prop:phaseA}, \ref{prop:phaseA2},
\ref{prop:phaseA3} hold computationally at $n\in\{9,10,11\}$.
\end{theorem}
...
At $n=10$: rank sequence $20\to 5\to 1$, support
collapsing to $S_3^{(10)}$.  At $n=11$: rank sequence $30\to 6\to 1$,
support $S_3^{(11)}$.  In all cases, the intermediate
$\widetilde{W}_j$ supports match the predicted shifting window
pattern.
\end{proof}
...
\item $a=3$, $n\in\{9,10,11\}$ (this writeup, computationally;
