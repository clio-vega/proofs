...
\item[\textbf{(C2)}] \textbf{\cite[Thm.~5.2]{Q149} is superseded and must not be quoted as the
  classification.}  It describes only the nowhere-zero part of the locus.  Its
  \texttt{lem:nonvanish} (=Lem.~8.2), ``a nonzero commuting weight vanishes nowhere'', is
  \textbf{false} for $\gcd(e,f)\ge3$ \cite[Cor.~1.4]{Q150c1}.  Every statement lifted from
  \cite[\S8]{Q149} must be checked against \cite[Thm.~1.3]{Q150c1} first.  Labels at risk in
  \cite{Q149}: \texttt{lem:flip} (8.1), \texttt{lem:nonvanish} (8.2), \texttt{lem:const}
  (8.3), \texttt{lem:char} (8.4), \texttt{prop:e1} (8.5).
