...
Let $P$ be such a path. Index its steps by $t=y-x$, where $(x,y)$ is the starting point of the
step; this is well defined, since every up step and every left step increases $y-x$ by exactly
$1$. Let $Y_P(t)$ be the $y$-coordinate at the start of step $t$, and let $A(P)\subseteq\Z$ be
the set of indices of the up steps. Since $Y_P$ increases by $1$ exactly at the up steps, for
each $i\in\Z$ there is a unique up step $t$ with $Y_P(t)=i$; set
\[
  x_i(P)\;:=\;\text{the unique }t\in A(P)\text{ with }Y_P(t)=i
  \qquad(\text{``the bead at height }i\text{''}).
\]
...
\begin{proof}
An up step sends $(x,y)\mapsto(x,y+1)$ and a left step sends $(x,y)\mapsto(x-1,y)$, so in both
cases $y-x$ increases by $1$; the steps of $P$ are therefore indexed by $\Z$, step $t$
beginning at the unique point of $P$ on the line $y-x=t$. The shift $(m-n,m)$ sends
$y-x\mapsto y-x+n$ and $y\mapsto y+m$, whence $x_{i+m}=x_i+n$; and $x_i$ is strictly
increasing in $i$ because $Y_P$ is weakly increasing in $t$. So $(x_i(P))\in\cyl^{n,m}$.
Conversely $(x_i)$ recovers $A(P)=\{x_i\}$ and $Y_P$ (by $Y_P(t)=\min\{i:x_i\ge t\}$), hence
recovers $X_P=Y_P-t$ and so $P$ itself.

For (i)--(iii) it suffices to verify the statements in a bounded window of rows: all three
conditions are local --- each involves only finitely many rows and columns at a time --- and
both sides are invariant under the shift. In a bounded window of $R$ consecutive rows a
cylindric shape is the boundary path of an ordinary partition $\lambda$, the beads $x_i$ for
$i$ in that window are $\lambda_{(i)}+i+\mathrm{const}$ where $\lambda_{(i)}$ is the length of
row $i$ (rows counted upward), i.e.\ the $\beta$-set of $\lambda$ read from the bottom, and
(i)--(iii) become the classical statements $\mu\subseteq\lambda\iff\mu_{(i)}\le\lambda_{(i)}$;
$|\lambda/\mu|=\sum_i(\lambda_{(i)}-\mu_{(i)})$; and $\lambda/\mu$ is a horizontal strip iff
consecutive rows interlace, which in $\beta$-coordinates is exactly \eqref{eq:hstrip}
(Macdonald I.(1.7)).
\end{proof}
...
may be checked in a bounded window where a cylindric shape is the boundary path of an ordinary
partition, is correct but compact. A referee would want the window bookkeeping spelled out:
the explicit constant relating $x_i$ to $\lambda_{(i)}+i$ in a given window, and the check that
the three conditions are stable as the window slides. I regard this as routine, but it is the
only step not written at full length, and it is the step at which a convention error would
produce a correct theorem about the wrong object. That is why the anchor and the positive
control of \S\ref{sec:verification} were run before anything else. I also note that my first
draft of Proposition~\ref{prop:dictionary} claimed a bijection between cylindric shapes and
periodic \emph{bead sets}; that is false --- a bead set determines the path only up to diagonal
translation --- and the corrected statement indexes beads by height, which is what carries
Postnikov's shift $d$ (Remark~\ref{rem:shift}).
