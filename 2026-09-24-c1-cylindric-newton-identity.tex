...
\small
\textbf{Provenance.}\quad The mathematics of this note is that of commit
\texttt{clio-vega/proofs@48f9ad0} (24 September 2026, PROVE cycle 1).  The only
later change is this provenance paragraph, added in the following commit; no
statement, proof, definition, numerical claim or range is altered by it.  If you
hold a build whose body differs from \texttt{48f9ad0}, this sentence is false and
the difference is not mine to vouch for.
\\[4pt]
\textbf{Status.}\quad
...
\\[4pt]
\textbf{Novelty is unchecked and no priority claim is made.}\quad
A quantum Murnaghan--Nakayama rule for $\QH(\Gr_{kn})$ may well be in the
literature; Bertram--Ciocan-Fontanine--Fulton's quantum rim-hook rule and
Morrison--Sottile's \emph{Murnaghan--Nakayama rules in Schubert calculus}
are the obvious candidates and \textbf{I have not read either}.  Nothing here
is claimed to be first.  What is claimed is that the proof below is a proof:
it derives Theorem~\ref{thm:newton} from Postnikov's quantum Pieri corollary
and a two-line count in the trace monoid of $\A_n$.
...
downstream changes.
\item[(H4)] \textbf{Novelty unchecked.}  See the status box.  In particular I
have not read Bertram--Ciocan-Fontanine--Fulton or Morrison--Sottile, and a
quantum Murnaghan--Nakayama rule for $\QH(\Gr_{kn})$ may be known; if so,
Corollary~\ref{cor:MN} is a second proof of a known rule and must be described
that way.  No sentence here claims priority.
