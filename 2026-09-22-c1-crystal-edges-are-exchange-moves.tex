...
\textbf{Commit:} \texttt{clio-vega/proofs@278ccb0} \\
\end{tabular}\\[2pt]
...
no $x$ is admissible, over every instance of the reduction of \cite[\S3.4]{MS}), and let
...
Morse and Schilling \cite[Def.~3.2]{MS} define $\et$ on such a pair after fixing
...
all of it; the exchange move has none. The implementation of \cite[\S3.2, Def.~3.2]{MS} used
...
We use the block model of \cite[\S2]{Clio2}: for $S\subsetneq\Z/n$, identified with its
...
the unpaired elements of $S$ and of $T$. Then \cite[Def.~3.2(i)]{MS}
\[
...
Morse--Schilling handle this in \cite[\S3.4]{MS}: they bracket $i\in S$ with $i+1\in T$, show
(their existence lemma for Case-(3) indices, Lemma~3.13) that some index is in ``Case (3)'', i.e.\ lies in a maximal block
\[
...
whether the offset $t$ of \cite[Def.~3.2(i)]{MS} is computed against $\cont(u)$ or against the
...
\paragraph{Instrument validation.} The implementation of \cite[\S3.2 (the $uv$-pairing)]{MS} reproduces all
three pairings printed in \cite[Ex.~3.1]{MS} exactly, including the pair labels
($n=14$, $x=10$: pairs $(5,6),(2,4),(12,13)$, $L_1=\{8,9\}$; $n=14$, $x=3$: pairs
$(12,13),(9,11),(8,0),(5,6)$; $n=5$, $x=2$: pair $(0,1)$), and the implementation of
\cite[Def.~3.2(i)]{MS} reproduces all three $\et$-images printed in \cite[Ex.~3.4]{MS}
...
\cite[Def.~3.2]{MS}: agreement on $9{,}093$ of $9{,}093$ additive pairs with $\XX\ne\emptyset$.
...
\paragraph{Consumed.} \S3.2, Definition~3.2 and \S3.4 of \cite{MS}, read at source; Lemmas
...
\cite[Thm.~3.5]{MS} (that $B(w)$ is a crystal): Theorem~\ref{thm:contain} is a statement about
...
\item[(G2)] The reading of ``$t$ maximal'' and of ``proceed with $x=b$'' in \cite[\S3.4]{MS} is
