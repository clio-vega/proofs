...
\textbf{Commit:} \texttt{clio-vega/proofs@da7f053} \\
\end{tabular}\\[2pt]
\textbf{Title:} Every Morse--Schilling crystal edge is an exchange move, and the
exchange move is strictly more general.\\[1pt]
\emph{The commit named above contains the mathematics of this note. Since then the
mathematics has not changed, and there have been exactly two non-mathematical edits, both to
the Gaps section: on 2026-09-22 c2 the (G1) clause ``which need not be additive'' was withdrawn
as false, with the replacement conjecture and its census recorded in its place
(\texttt{clio-vega/proofs@eb9d081}); on 2026-09-23 c1 that conjecture was proved and (G1) was
closed, as recorded in the (G1) entry itself. This sentence says what changed by kind, not that
nothing did --- a claim that nothing below has changed is a claim about every line below it,
and is refuted by any later edit anywhere.}
\end{center}
...
$\XX\ne\emptyset$: the conclusion is uniform over all two-factor affine factorisations. The
first two bullets above were recorded here as computational; they were \textbf{proved} on
2026-09-23 --- see (G1) in \S\ref{sec:gaps}. The third (strictness, failing in $7$ pairs at
$n=7$) is an existence statement certified by its witnesses and remains computational.
...
$(S\setminus\{b-1\},T\setminus\{b\})$, and Lemma~\ref{lem:step1} is not available for it.
I do not know whether Theorem~\ref{thm:contain} holds there for every choice of Case-(3)
index; it holds in all $1636$ instances with $n\le7$.

\emph{Correction and route, 2026-09-22 c2 (peer-review session,
\texttt{reviews/2026-09-22-c2-q223-neyman.md} \S6).} The first version of this item asserted
that the reduced pair ``need not be additive''. That assertion was never checked and appears to
be \emph{false}: over every additive $(S,T)$ with $\XX=\emptyset$ and every Case-(3) index
$b$, the reduced pair is additive in all $6786$ instances with $3\le n\le 8$, with no
exception. Dropping the Case-(3) hypothesis gives the clean statement

\smallskip
\emph{Conjecture (bracketed-pair deletion).} If $(S,T)$ is additive, $i\in S$ and $i+1\in T$,
then $(S\setminus\{i\},T\setminus\{i+1\})$ is additive.
\smallskip

\noindent verified in $82\,044$ of $82\,044$ instances for $3\le n\le 8$. The test is not
vacuous: the unbracketed analogue --- additivity hereditary under $S'\subseteq S$,
$T'\subseteq T$ --- is \emph{false}, with smallest witness $n=3$, $S=\{0\}$, $T=\{0,1\}$,
$S'=T'=\{0\}$. Proving the conjecture makes Lemma~\ref{lem:step1} applicable to the reduced
pair and closes (G1), upgrading \S\ref{sec:emptyX} from computed to proved. Recorded as
question Q232. Code:
\texttt{reviews/code-2026-09-22-c2/g1\_reach.py}, \texttt{g1\_lemma.py},
\texttt{g1\_lemma2.py}.

\emph{CLOSED, 2026-09-23 c1.} The conjecture is proved --- in the stronger form with no
bracket hypothesis: if $(S,T)$ is additive and $i\in S$, then $(S\setminus\{i\},T\setminus\{i+1\})$
is additive, whether or not $i+1\in T$. See Clio, \emph{Deleting a bracketed pair preserves
length-additivity}, 23 September 2026, \texttt{clio-vega/proofs@b2b0297}, Theorem 3.1 and
Corollary 3.2. That note also supplies the step Q232 alone does \emph{not} supply, namely the
transport of the resulting move from the reduced pair back to $(S,T)$ (its Theorem 6.2),
including a side condition $|T|\le n-2$ which genuinely fails on $894$ of the $6786$ in-range
instances and is discharged by showing those are exactly the instances producing no move.
Consequently \S\ref{sec:emptyX} is \textbf{proved}, not computed, for both of its
containment assertions, and the reading ambiguity in (G2) between the two constructions I
could write down is proved immaterial. What remains of (G2) is exegetical only; see the
remark following Theorem 6.2 there.
