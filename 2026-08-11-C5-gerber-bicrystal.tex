...
\begin{lemma}[Validity dictionary]\label{lem:Sr}
Let $\mu = e\lambda$, $L = \ell(\lambda)$, with $0$-indexed rows and the
conventions $\mu_{-1} = +\infty$, $\mu_L = 0$. For $0 \le r \le L$ put
\[
  S_r \;:=\; [\, \mu_{r-1} > \mu_r \,] \;=\; [\, \lambda_{r-1} > \lambda_r \,].
\]
Then:
\begin{enumerate}
  \item $\mu$ has an addable node in row $r$ $\iff$ $S_r$;
  \item $\mu$ has a removable node in row $r-1$ $\iff$ $S_r$ \quad
        (for $1 \le r \le L$);
  \item when they exist, both nodes have residue $-r \bmod e$.
\end{enumerate}
In particular $S_0$ and $S_L$ are both \emph{true}, so case (a) below
supplies an unpaired $\Aa$ and case (c) a genuine pair.
\end{lemma}

\begin{proof}
(1) By definition an addable node in row $r$ exists exactly when row $r$ is
a strict step, i.e.\ $r = 0$ (where $\mu_{-1} = +\infty > \mu_0$) or
$\mu_{r-1} > \mu_r$; for $r = L$ this is $\mu_{L-1} > 0 = \mu_L$, true. All
three cases are the single statement $S_r$.

(2) By definition a removable node in row $r-1$ exists exactly when
$\mu_{r-1} > \mu_{r}$ --- for $r - 1 = L-1$ this reads
$\mu_{L-1} > 0 = \mu_L$, again true. \emph{This is literally the same
inequality between the same two consecutive parts as in (1).} The apparent
index shift is illusory: removability looks one row \emph{down}, addability
looks one row \emph{up}, and shifting the removable row from $r$ to $r-1$
cancels that difference exactly. No property of $e\lambda$ is used; the
coupling holds for any partition.

Since $\mu_{r-1} = e\lambda_{r-1}$ and $\mu_r = e\lambda_r$ and $e > 0$, the
inequality $\mu_{r-1} > \mu_r$ is equivalent to $\lambda_{r-1} > \lambda_r$.

(3) \emph{Here} divisibility by $e$ is used, and it is what makes the two
residues agree. The addable node of row $r$ sits at column $\mu_r \equiv 0
\pmod e$, so its residue is $\mu_r - r \equiv -r$. The removable node of row
$r-1$ sits at column $\mu_{r-1} - 1 \equiv -1 \pmod e$, so its residue is
$(\mu_{r-1}-1)-(r-1) \equiv -r$. Both are $\equiv -r$, hence both equal $i$
precisely when $r \equiv -i \pmod e$.
\end{proof}

\begin{remark}
Lemma~\ref{lem:Sr}(3) is the only place where $\mu = e\lambda$ (as opposed to
an arbitrary partition) is used, and it is used only to align the two
residues. The coupling (1)$\Leftrightarrow$(2) is completely general. This is
the precise sense in which ``inflate by $e$'' is what makes the signature
pair up: it does not create the pairing, it makes the two members of each
pre-existing pair carry the same residue.
\end{remark}
...
\medskip
\emph{Step 1b: the validity coupling.} Lemma~\ref{lem:Sr} is the
load-bearing fact of the whole argument: the addable node of row $r$ and the
removable node of row $r-1$ are governed by \emph{the same} boolean. If they
were governed by different booleans, lone $\Rr$ symbols would appear in the
signature word and the bound $\eps_i \le 1$ would fail. (This step was
asserted without proof in the first version of this note; the gap was
identified by Lyra in her review of 30 August 2026 and is closed here.)
...
By Lemma~\ref{lem:Sr} the residues match in these adjacent rows, and both
the addable-at-$r$ and the removable-at-$(r-1)$ existence conditions are the
single boolean $S_r = [\lambda_{r-1} > \lambda_r]$.

We split into cases, in each case reading Lemma~\ref{lem:Sr} at the relevant
$r$:
...
(b) \emph{$1 \le r \le L - 1$, $r \equiv -i \pmod e$.} By
Lemma~\ref{lem:Sr}, addable-at-$r$ and removable-at-$(r-1)$ are both present
if $S_r$ holds and both absent if it does not; there is no case in which
exactly one of them occurs. The two nodes sit in rows $r$ (deeper) and
$r-1$ (shallower), so in bottom-up order this contributes $\Aa\Rr$ --- and
otherwise nothing.

(c) \emph{$r = L$, $L \equiv -i \pmod e$.} Here $S_L$ is true
unconditionally ($\mu_{L-1} > 0 = \mu_L$), so by Lemma~\ref{lem:Sr} the
addable at the new row $(L,0)$ and the removable at row $L-1$ are both
present. This contributes $\Aa\Rr$ in bottom-up order.
