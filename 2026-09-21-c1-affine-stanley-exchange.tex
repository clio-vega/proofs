...
\textbf{Commit:} \texttt{clio-vega/proofs@22d1b22} \\
\end{tabular}\\[2pt]
\textbf{Title:} An exchange move for cyclically decreasing factorisations,
and the reduction of WZZ Problem 5.1 to a single hypothesis.\\[1pt]
...
\emph{To be unambiguous about what was omitted:} (H3) and (H4) are both \emph{proved} in
\cite{Clio2026} --- they are Lemma~3.2 and Lemma~5.2(3) there --- and nothing in that paper is
missing or unjustified. What was omitted is their appearance in the informal two-item summary of
the template, and the omission matters only here, where the template is transported to a setting
in which they must be re-established rather than inherited.
...
\begin{rem}[Prior appearances of special cases]\label{rem:prior}
Special cases of Theorem~\ref{thm:exchange} appear as the commutation and chute moves of
\cite[Lem.~23]{Denton}, under the additional hypothesis that the supports be cyclic
intervals; the closely related crystal operator $\tilde e_1$ of
\cite[Def.~3.2, Lem.~3.8]{MorseSchilling} realises a canonical choice of the move. Three
points of accounting.

\emph{(i) The containment is strict and one-directional.} Implementing the Morse--Schilling
pairing and comparing edge sets exhaustively for $3\le n\le 7$, every $\tilde e_1$ edge is a
move admitted by Theorem~\ref{thm:exchange}, and at $n=7$ there are moves of
Theorem~\ref{thm:exchange} that are not $\tilde e_1$ edges. The novelty here is therefore
\emph{the removal of the $x$-dependent pairing}, not the existence of a move.

\emph{(ii) These are not independent witnesses.} \cite{MorseSchilling} cite \cite{Denton},
and both rest on \cite{Lam2006}; they are independent \emph{proofs}, not independent sources.

\emph{(iii) Coverage.} \cite[Lem.~23]{Denton} does not cover all instances of
Theorem~\ref{thm:exchange}: the smallest uncovered case is $n=3$, $S=\{0,1\}$, $T=\{1\}$,
which falls under that paper's Lemma~5 instead.

The one direction that matters for \S\ref{sec:consumes}: nothing in this paper is
\emph{deduced} from either reference. They are recorded as prior art.
\end{rem}
...
\begin{rem}[The two-factor case is known]\label{rem:twofactor}
Corollaries~\ref{cor:interval} and~\ref{cor:pal} together say that for a two-factor
decomposition the set $\mathcal F(v)$ of admissible first-factor lengths is a full integer
interval symmetric about $\ell(v)/2$. That conclusion is already available from
\cite[\texttt{theorem.two factor}]{MorseSchilling}, which states, unconditionally and with no
further hypothesis, that for any $w\in\Saff$ admitting an affine factorisation into two
factors the set of such factorisations carries the structure of a $U_q(\mathfrak{sl}_2)$-crystal;
the string property of an $\mathfrak{sl}_2$-crystal is exactly the interval-plus-palindrome
statement. I record this as a prior appearance. Two differences remain: the proof given here
does not use Hypothesis~\ref{hyp:sym} for Corollary~\ref{cor:interval}, and it is the
multi-factor case (Hypothesis~\ref{hyp:h4}) that is wanted, where the same authors observe
(l.~1828--34) that ``such a theorem will not exist without generalizing the notion of
crystal''. That observation is the reason the argument of this paper is greedy rather than
crystal-theoretic.
\end{rem}
...
$\mathcal P_{\widehat\lambda}\cap\Z^r$. For the two halves of that sentence, separately:
M-convexity is \cite[Lem.~6.2 and Prop.~6.3]{Clio2026}, self-contained polymatroid theory proved
from the exchange axiom; the identification with $\mathcal P_{\widehat\lambda}\cap\Z^r$ is
\cite[Thm.~8.1 and Prop.~8.3]{Clio2026}. (Earlier builds of this note cited ``\S6'' for both.
\S6 does not prove the second half --- that was the content of Gaps item 2 of \cite{Clio2026},
closed on 25 September 2026 by the new \S8, whose Lemma~8.2 and Proposition~8.3 are stated for an
arbitrary convex $S_r$-stable set and so apply here verbatim. Numeric citations to \S6 of builds
before that date are shifted by the insertion of Lemma~6.2; cite by label.)
\end{proof}
...
\item \textbf{\cite[Lem.~23]{Denton} and \cite[Lem.~3.8]{MorseSchilling}}: \emph{not used}.
They are recorded in Remark~\ref{rem:prior} as prior appearances of special cases; no step
below is deduced from either.
\item \textbf{\cite[Lem.~4.1, \S6 and \S8]{Clio2026}}, all by the present author and all
self-contained: the Robin Hood step for the dominance order, the passage from an $S_r$-stable
dominance ideal with a maximum to M-convexity (\S6), and the identification of the lattice
points of the permutahedron (\S8, which re-proves the direction of Rado's theorem it needs from
the Robin Hood step rather than citing it).
\end{itemize}
...
\item \textbf{Hypothesis~\ref{hyp:sym} is cited, not proved here. \textsc{Source-depth gap
closed 21 September 2026 (c2).}} It is attributed to \cite{Lam2006}. At the time of writing
the author had not read that paper at first hand and recorded the fact here. \cite{Lam2006}
has since been read at source (arXiv long version): the statement is \texttt{thm:sym} at
l.~626, \emph{``The generating functions $\widetilde F_w(X)\in\Lambda_n$ are symmetric''}, and
it follows there from \texttt{prop:commute} at l.~655, \emph{``the elements $h_k(u)$ for
$k\in[0,n-1]$ commute''}, proved by an explicit local bijection on cyclic intervals in the
affine nilCoxeter algebra. The proof is elementary and unconditional; in particular it does
not touch \cite[Cor.~8.5]{Lam2008}. The hypothesis is therefore now a cited theorem read at
first hand, and results resting on it are no longer capped below \emph{proved}. It is used
only for Corollary~\ref{cor:pal}; Theorem~\ref{thm:exchange} and Corollary~\ref{cor:interval}
--- the new content --- do not depend on it. It would still be of interest to prove
Corollary~\ref{cor:pal} directly, which is
...
\begin{rem}[Added 21 September 2026 (c2): Hypothesis~\ref{hyp:h4} on the 321-avoiding class]
Hypothesis~\ref{hyp:h4} is now \emph{proved} for every 321-avoiding $w$, by a different route:
see \cite{Clio2026c}, Corollary 6.3. That argument transports the cylindric support theorem of
\cite{Clio2026} across Lam's theorem that a 321-avoiding $w$ has $\widetilde F_w$ equal to a
cylindric skew Schur function, and produces the simultaneous maximiser explicitly as the
weight of a greedy chain. Two further remarks, both from computations reported there.
First, the specific route abandoned in item~1 above --- a maximum of $A_t$ in the right weak
order --- fails only off the 321-avoiding class: of the $178$ failing triples $(w,r,t)$ counted
above, \emph{none} has $w$ 321-avoiding, while all $410$ triples with $w$ 321-avoiding do have
a weak-order maximum. Second, this does not close Problem 5.1: the argument of
\cite{Clio2026c} imports \cite[\texttt{thm:321}]{Lam2006} in place of
\cite[Cor.~8.5]{Lam2008}, and so is not a proof from \eqref{eq:def}.
\end{rem}
...
\bibitem{Clio2026c} Clio. \emph{M-convexity and the Newton polytope of affine Stanley
  symmetric polynomials on the 321-avoiding class, without dual $k$-Schur positivity}.
  \texttt{projects/proofs/2026-09-21-c2-affine-stanley-321-avoiding.tex}, 21 September 2026.
\bibitem{Lam2006} T.~Lam. \emph{Affine Stanley symmetric functions}. Amer. J. Math.
  ...
  \texttt{thm:sym} l.~626, \texttt{prop:commute} l.~655, \texttt{thm:monomial} l.~945,
...
\bibitem{MorseSchilling} J.~Morse and A.~Schilling. \emph{Crystal approach to affine Schubert
  calculus}. arXiv:1408.0320. (Read at LaTeX source: Def.~3.2, Lem.~3.8, Def.~4.4(i), and
  \texttt{theorem.two factor} at l.~1822--26.)
\bibitem{Denton} T.~Denton. \emph{Canonical decompositions of affine permutations, affine codes,
  and split $k$-Schur functions}. Electron. J. Combin. \textbf{19}(4) (2012), \#P19;
  arXiv:1204.2591. (Lemma~23 read at source and checked against the published version.)
