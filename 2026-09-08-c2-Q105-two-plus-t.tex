...
\textbf{Corresponds to:} \texttt{clio-vega/proofs@69f93c6} (paper, code, and the seven
...
\begin{remark}[annotation added 2026-09-09 after reading Day~180 at source]
The quotation above is verbatim (Day~180 summary block, Ask~2) and the refutation in
Corollary~\ref{cor:S2} stands unchanged.  Two corrections to the \emph{framing} of the
preceding remark, both in Rick's favour, recorded rather than silently edited.
(i)~The ``$75\%$ stated confidence'' is attached to his \emph{conclusion} --- ``Ask 2 (N--R
$E(-u/t)$ is the dictionary?)'' --- not to the Wick sentence, which is the \emph{premise}
offered as evidence for it (``the shape matches: \dots\ \emph{so} N--R's $E(-u/t)$ IS a
plausible candidate'') and which carries no hedge at all.  The preceding remark therefore
both understates how firmly the refuted sentence was asserted and implies a hedge that is
not on it.
(ii)~The locus/object decomposition is \emph{his}: his summary reads ``If it is, then your
$ts=1$ vanishing hyperbola is the twist-invariance locus'', and his \S4 separates the locus
(item~2) from the object (item~3) as numbered claims.  He arrived at that split first.
His \S4 as a whole is moreover explicitly provisional --- titled ``(probably)'', opening
``I don't have Necoechea--Rozhkovskaya on disk'', with the key equation marked ``(guess,
needs check)'' --- so no committed belief is being corrected here, only an unhedged premise.
Reviewed in \texttt{clio-vega/rick-review@3416822}.
\end{remark}

\begin{remark}[what N--R actually says, read at source 2026-09-09]
Rick's \S4 asks whether \cite{NR} contains $H_t(z)=E(-z/t)\psi(z)E(-z/t)^{-1}$.  It does not.
At source lines 392--394 the twist is \emph{left multiplication}, and it is \emph{asymmetric}:
$\Psi^{+}(u)=E(-u/t)\Phi^{+}(u)$ but $\Psi^{-}(u)=H(u/t)\Phi^{-}(u)$ --- not a twist and its
inverse.  The twisted anticommutator (Prop.\ \texttt{ferm\_twisted}, ll.~398--405) is
$(1-vt/u)\Psi^{+}(u)\Psi^{-}(v)+(1-ut/v)\Psi^{-}(v)\Psi^{+}(u)=\delta(u,v)(1-t)^{2}$: the
constant is $(1-t)^{2}$, and the relation is single-parameter.  The two-parameter exchange
relation is Proposition~\ref{prop:zeropole} above, at $a=c=1$, $b=t$, $d=s$.
\end{remark}

\section{Verification}\label{sec:verif}

All primary computations for this paper were run on two engines written this session:
\texttt{clio-vega/proofs@69f93c6:2026-09-08-c2-Q105-code/abacus.py} (Maya sets and bead
