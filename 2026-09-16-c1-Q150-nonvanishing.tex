...
\cite[Lemma 7.3]{Q149} (``\textsf{lem:nonvanish}'') asserts: if $[R_e^{W},R_f^{\bar W}]=0$
...
=\rho\otimes\sigma$.}  So the $\gcd$ in \cite[Thm.~1.4]{Q149} is not an accident of the
...
$\tau$ does.  Consequently \cite[Lemma 7.3]{Q149} is
\[
...
\begin{remark}
Proposition~\ref{prop:1B} supersedes \cite[Lemma 7.1]{Q149} (\textsf{lem:flip}).  The four
...
\cite[Cor.~1.5]{Q149}, now with no appeal to \cite[Lemma 7.3]{Q149} --- and, remarkably, with
no appeal to the two-bead sector either.
\end{proof}

\begin{remark}
Corollary~\ref{cor:coprime} also reproves \cite[Prop.~7.7]{Q149} (\textsf{prop:e1}, the case
...
particular \cite[Lemma 7.3]{Q149} is false for every such $(e,f)$, and with it
\cite[Theorem 1.4]{Q149}\,(necessity).  This is a sufficiency statement only: it
...
That this contradicts \cite[Lemma 7.3]{Q149} is immediate.  It contradicts
\cite[Thm.~1.4]{Q149}(necessity) as well: case (c) there asserts
...
given in the ribbon-shape indexing of \cite[Lemma 2.2]{Q149} by
\[
...
$\alpha(1,0)=(3-1,\,1-0)=(2,1)$ by \cite[Lemma 2.2]{Q149}; and $W(1,0)=(-1)^{1}\cdot1=-1$.
...
\cite[Thm.~1.4(c)]{Q149}: $\sigma=\alpha\,\sigma^{\gamma}_e$, $\bar\sigma=\beta\,
...
So \cite[Thm.~1.4]{Q149} is not wrong about the solutions it lists; it is incomplete.  Its
...
\texttt{direct.py} is written from \cite[Conv.~2.2]{Q149} and shares no algebra with
...
\item Theorem~\ref{thm:gcdle2}: nonvanishing for $\gcd(e,f)\le2$, hence
  Corollary~\ref{cor:coprime}, hence \cite[Cor.~1.5, Prop.~7.7]{Q149} unconditionally.
...
\bibitem{Q149} Clio, \emph{Shape weights on ribbons: the commuting locus is the
  ...
  Numbering used above: Conv.~2.1 $=$ \textsf{conv:sigma}, Conv.~2.2 $=$ \textsf{conv:def},
  Lemma 2.2 $=$ \textsf{lem:dict}, Lemma 3.1 $=$ \textsf{lem:realise},
  Lemma 4.1 $=$ \textsf{lem:sectors}, Lemma 4.2 $=$ \textsf{lem:onebead},
  Lemma 4.3 $=$ \textsf{lem:twobead}, Thm.~1.4 $=$ \textsf{thm:main},
  Cor.~1.5 $=$ \textsf{cor:coprime}, Conv.~5.1 $=$ \textsf{conv:gamma},
  Lemma 7.1 $=$ \textsf{lem:flip}, Lemma 7.3 $=$ \textsf{lem:nonvanish},
  Prop.~7.7 $=$ \textsf{prop:e1}.
