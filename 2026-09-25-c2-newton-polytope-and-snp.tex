...
The first sentence of the proof of Proposition 6.2 of~\cite{note1} asserts an
...
the hypothesis of the SNP argument of \S\ref{sec:snp}; and it is the one step of
Proposition 6.2 that is \emph{not} formalised in Lean, the formalisation working
throughout in the subset form (Remark 6.3 of~\cite{note1}). The same bridge is
...
\item \textbf{Lemma~\ref{lem:JJ} is still not formalised.} Remark 6.3
of~\cite{note1} records that the Lean development works throughout in the subset
form and that the identification with the sorted form is unformalised. This note
supplies a written proof, which upgrades the status of the paper but not of the
Lean record; \texttt{polymatroid-exchange} therefore stays below
\texttt{lean-verified} until Lemma~\ref{lem:JJ} is transported. It is now a
self-contained statement about $\N^{\ell}$ with no cylindric content, so it is a
reasonable formalisation target.
...
References are to the build of 25 September 2026 (post-erratum
\texttt{clio-vega/proofs@a6c83ed}), in which Lemma 4.1 is \texttt{lem:hlp},
Proposition 4.2 is \texttt{prop:ideal}, Corollary 3.4 is \texttt{cor:bk},
Proposition 5.5 is \texttt{prop:max}, Proposition 6.2 is
\texttt{prop:perm-mconvex} and Theorem 7.1 is \texttt{thm:main}. Theorem numbers
here are taken from that build's \texttt{.aux} file, not from recall.
