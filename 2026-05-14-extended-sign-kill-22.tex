...
\subsection{Why this is plausible to extend}

The argument generalizes mechanically to any shape $\lambda$ where:
\begin{enumerate}[label=(\arabic*),topsep=0.3em,itemsep=0.1em]
\item $\dim B$ is known and the explicit support of a basis of $B$
  is known.
\item The support SYTs share a common ``top-of-column-$1$'' pattern,
  i.e., there is some $i_0$ such that every support SYT has
  $T(i, 1) = i$ for $i \le i_0$.
\item There is a sharpness witness at $j = i_0 + 1$, again following
  from the support description.
\end{enumerate}
For $\hatl = (3, 2)$, the May-20 paper identifies $B$ as the image of
the three-branch decomposition under $\Phi_X$; this gives a candidate
support structure that should allow an analogous argument for
$E_j^-$ at $j \le q - 2$.  We sketch the extension as future work.
