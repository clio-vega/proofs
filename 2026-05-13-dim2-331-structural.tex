...
\[
   ...
   \;=\; \dim B^{(\mu_1)} \;=\; 2
\]
(using $\Phi_*$ injective by SRD-331 Lemma~2.4 and Theorem~\ref{thm:mu1}). This is the
...
$\mu_1$-dim}.
\item For the $(4, 3, 1^q)$ family ($r = 2$, conjecturally $\dim = 5$):
the present paper closes one ingredient but \emph{not} the full
lower bound; see Remark~\ref{rem:431-partial} below.
\end{itemize}

\begin{remark}[Partial application to $(4, 3, 1^q)$]\label{rem:431-partial}
In the $r = 2$ reduction formula for $\lambda = (4, 3, 1^q)$
(\texttt{2026-05-13-4-3-1n-lower-bound-analysis.tex}),
$B^{(\lambda)} = \mathcal{O}_\lambda
[\Phi_A^\lambda(B^{(\mu_A)}) + \Phi_B^\lambda(B^{(\mu_B)})]$,
the present paper provides one ingredient: $\mathcal{O}_\lambda$ acts
injectively on each individual $\Phi_X(B^{(\mu_X)})$ (each lies in
$E_{n-1}^+$ and adjacent-injectivity propagates). Hence the lifted
inner bases $\{F_A^1, F_A^2\}$ and $\{F_B^1, F_B^2, F_B^3\}$ are
linearly independent within each $\Phi_X$-class.

However, this is \emph{not} sufficient to close the full
$5$-vector independence. The lower-bound-analysis paper's Theorem~3.2
reduces the problem to projected versions
(a) $\{\pi^{(c_1)}_n F_A^i\}$ and (b) $\{\pi^{(c_2)}_n F_B^j\}$. The
projected vectors are not in $E_{n-1}^+$ (the seminormal projection
breaks the $+q$-eigenvector structure), so adjacent-injectivity does
not directly apply --- a cross-class linear combination could carry
$c_3$-overlap invisible to either projection alone. The honest status
of the $(4, 3, 1^q)$ lower bound is unchanged: tracer-pair Hoefsmit
remains the open ingredient.
\end{remark}
