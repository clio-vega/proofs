...
\textbf{Title:} The cross-rank commutator $[R_e(t),R_f(t)]$ in closed form\\
\textbf{Corresponds to:} \texttt{clio-vega/proofs@a26acae}\\
\textbf{Annotation, 8 September 2026:} the clause ``$e=f\Rightarrow$ the two-bead element
is $0$'' in Thm.~2.3(2) is a \emph{one-parameter} statement.  It is true as stated (both
operators at the same $t$) and \emph{false} in the two-parameter generality: at $e=f$ there
are two legal assignments whose indices are negatives of one another, and they cancel only
on $s=t$.  See \texttt{2026-09-07-c2-Q96-order-over-sublattice.tex} \S4.\\[4pt]
\textbf{Annotation, 8 September 2026 (second pass, self-review
\texttt{clio-vega/rick-review@8223c88}, finding F2):} the annotation above is itself too
strong on two counts.  The two legal assignments cancel not only on $s=t$ but also on
$ts=1$; and for $e=f\in\{1,2\}$ they cancel \emph{identically}, because $Q=P-k$ is forced
there, so for those two ranks the clause ``$e=f\Rightarrow$ the two-bead element is $0$''
of Thm.~2.3(2) is true in the two-parameter generality as well.  The correction stands for
$e\ge3$ only.  Separately, the lead clause of Cor.~5.1 (``for $e,f\ge2$ the two-bead part of
$[R_e(t),R_f(t)]$ is nonzero'') needs $e\neq f$: at $e=f$ the commutator is $0$, as
Thm.~2.3(2) says.  Thm.~2.3 itself is unaffected.
}}
\end{center}
\bigskip
...
Two gaps in the first draft are closed in \S\ref{sec:closed}: $\Psi_{e,f}\ne0$ for
\emph{all} $e,f\ge2$, by a uniform witness; and the three-bead sector of
$[R_e,[R_f,R_g]]$ factorises as
$t^{P}\bigl(t^{\kappa_{32}}-t^{-\kappa_{32}}\bigr)
 \bigl(t^{\kappa_{21}+\kappa_{31}}-t^{-(\kappa_{21}+\kappa_{31})}\bigr)$,
which makes the bead-$3$ statement uniform.  What remains open is stated in
\S\ref{sec:gaps}; the honest headline there is that this session had no network, so
the $W_{1+\infty}$ / Bloch--Okounkov novelty question is \emph{unchecked} and nothing
here claims priority.
...
sites.  See \S\ref{sec:ver}.  Corollary~\ref{cor:three} upgrades this to a uniform
statement with a proof, for infinitely many triples $(e,f,g)$.
\end{proof}
...
Corollaries~\ref{cor:unbdd} and \ref{cor:three} it is not finitely graded by
bead-number.
\end{remark}
...
\section{Closing the two gaps: uniform witnesses}\label{sec:closed}

Both gaps I first recorded turned out to be shallower than they looked, for the same
reason: Theorem~\ref{thm:matrix} determines a matrix element by a \emph{single}
configuration, so no cancellation is available to hide behind.

\begin{proposition}[$\Psi_{e,f}\ne0$ for all $e,f\ge2$]\label{prop:psiwitness}
Let $e,f\ge2$ with $e\ne f$, take $\lambda=\varnothing$ (so $M=\Z_{<0}$), and let $\mu$ be
the partition with $M(\mu)=\bigl(\Z_{<0}\setminus\{-1,-f\}\bigr)\cup\{e-1,0\}$.  Then
\[
  \bigl\langle s_\mu,\,[R_e(t),R_f(t)]\,s_\lambda\bigr\rangle=t^{\,f}-t^{\,f-2}\ \ne\ 0 .
\]
In particular $\Psi_{e,f}\ne0$, and the two-bead part of $[R_e,R_f]$ is nonzero, for every
$e,f\ge2$.
\end{proposition}

\begin{proof}
Put $b=-1$, $c=-f$.  Then $b,c\in M$ (both are negative, using $f\ge1$);
$b+e=e-1\ge1$ and $c+f=0$ are both $\notin M$; and the four sites $-1,-f,e-1,0$ are
pairwise distinct because $f\ge2$ (so $-f\ne-1$) and $e\ge2$ (so $e-1\ge1$, hence
$e-1\notin\{-1,-f,0\}$).  So this is a legal two-bead configuration with target $M(\mu)$.

Its interference index: $\alpha'=[\,c<b<c+f\,]=[-f<-1<0]=1$ since $f\ge2$, and
$\beta'=[\,c<b+e<c+f\,]=[-f<e-1<0]=0$ since $e-1\ge1>0$.  Hence
$k^{e,f}_{b,c}=-1$.  Also $P=\#\bigl(M\cap(-1,e-1)\bigr)=0$ (that interval contains only
non-negative integers) and $Q=\#\bigl(M\cap(-f,0)\bigr)=f-1$.

By Theorem~\ref{thm:matrix}(2) --- and this is the point --- the pair $(b,c)$ is
\emph{determined} by $M$ and $M(\mu)$ when $e\ne f$, so the matrix element is this single
term and nothing can cancel it:
$t^{P+Q}(t^{-k}-t^{k})=t^{f-1}(t-t^{-1})=t^{f}-t^{f-2}$.
Since $\Phi_{e,f}$ has bead-number $1$ it contributes nothing here, so
$\Psi_{e,f}\ne0$.
\end{proof}

\begin{remark}
For $(e,f)=(2,3)$ this is the witness of Corollary~\ref{cor:noclose}:
$\mu=(2,2,1)$ and the coefficient is $t^{3}-t$.  Verified against Engine~A for all
$2\le e,f\le7$, $e\ne f$ ($30/30$).
\end{remark}

We now do the same one level up.  Fix three legal moves $m_i=(b_i,g_i)$, $i=1,2,3$, on
$M$, with $(g_1,g_2,g_3)=(e,f,g)$ and with the six sites $b_i,b_i+g_i$ pairwise distinct.
Write $P_i=\#\bigl(M\cap(b_i,b_i+g_i)\bigr)$ and
\[
  \kappa_{ij}=\bigl[\,b_i+g_i\in(b_j,b_j+g_j)\,\bigr]-\bigl[\,b_i\in(b_j,b_j+g_j)\,\bigr],
\]
the change move $i$ makes to the statistic of move $j$.  By Lemma~\ref{lem:kappa},
$\kappa_{ij}+\kappa_{ji}=0$ (the two degenerate cases $b_i=b_j$ and $b_i+g_i=b_j+g_j$ are
excluded by distinctness of the six sites).

\begin{proposition}[the three-bead sector factorises]\label{prop:three}
Let $M'$ satisfy $|M\triangle M'|=6$.  Then
\[
 \bigl\langle M'\bigl|\,[R_e,[R_f,R_g]]\,\bigr|M\bigr\rangle
 =\sum t^{\,P_1+P_2+P_3}\Bigl(t^{\kappa_{32}}-t^{-\kappa_{32}}\Bigr)
   \Bigl(t^{\,\kappa_{21}+\kappa_{31}}-t^{-(\kappa_{21}+\kappa_{31})}\Bigr),
\]
the sum over the assignments of the three beads of $M\setminus M'$ to the three
displacements $e,f,g$ that carry them onto $M'\setminus M$.  Each summand vanishes if and
only if $\kappa_{32}=0$ or $\kappa_{21}+\kappa_{31}=0$.
\end{proposition}

\begin{proof}
A product of three one-bead moves reaches a target at Hamming distance $6$ only if no two
of them contract, i.e.\ the three moves are as above; the assignment of displacements to
beads is what the sum ranges over.  All six sites being distinct, every ordering of the
three moves is legal: move $j$ finds $b_j$ still occupied and $b_j+g_j$ still empty
whatever else has happened.  Moreover, when move $j$ is performed after the moves in a
set $S$, its weight exponent is $P_j+\sum_{i\in S}\kappa_{ij}$, because each earlier move
$i$ removes a bead at $b_i$ and adds one at $b_i+g_i$, changing
$\#\bigl(\cdot\cap(b_j,b_j+g_j)\bigr)$ by exactly $\kappa_{ij}$, independently of the
other moves.  Hence performing the moves in the order $\sigma$ contributes
$t^{\,P_1+P_2+P_3+\sum_{i\prec_\sigma j}\kappa_{ij}}$.

Expanding, and recalling that $R_aR_bR_c$ applies $R_c$ first,
\[
 [R_e,[R_f,R_g]]=R_eR_fR_g-R_eR_gR_f-R_fR_gR_e+R_gR_fR_e
\]
corresponds to the four orders $(3,2,1),(2,3,1),(1,3,2),(1,2,3)$ with signs
$+,-,-,+$.  Their exponent corrections are, respectively,
\[
 \kappa_{32}+\kappa_{31}+\kappa_{21},\quad
 \kappa_{23}+\kappa_{21}+\kappa_{31},\quad
 \kappa_{13}+\kappa_{12}+\kappa_{32},\quad
 \kappa_{12}+\kappa_{13}+\kappa_{23}.
\]
The first two share $\kappa_{31}+\kappa_{21}$ and the last two share
$\kappa_{12}+\kappa_{13}$, so with $P=P_1+P_2+P_3$ the bracket equals
\[
 t^{P}\Bigl(t^{\kappa_{32}}-t^{\kappa_{23}}\Bigr)
      \Bigl(t^{\,\kappa_{21}+\kappa_{31}}-t^{\,\kappa_{12}+\kappa_{13}}\Bigr),
\]
and substituting $\kappa_{23}=-\kappa_{32}$, $\kappa_{12}=-\kappa_{21}$,
$\kappa_{13}=-\kappa_{31}$ gives the statement.  A factor $t^{a}-t^{-a}$ vanishes iff
$a=0$.
\end{proof}

\begin{corollary}[bead-number $3$ occurs, uniformly]\label{cor:three}
Let $e\ge2$, $f\ge3$, $g\ge e+2$ with $e,f,g$ distinct and $(e,g)\ne(2,f+1)$.  Take
$\lambda=\varnothing$ and the moves $b_1=-e$ (by $e$), $b_2=-1$ (by $f$), $b_3=1-g$ (by
$g$).  Then the assignment is unique, $\kappa_{32}=1$, $\kappa_{21}=-1$, $\kappa_{31}=0$,
and
\[
 \bigl\langle s_\mu,\,[R_e,[R_f,R_g]]\,s_\varnothing\bigr\rangle
 \;=\;-\,t^{\,e+g-5}\,(t^{2}-1)^{2}\ \ne\ 0,
\]
where $M(\mu)=\bigl(\Z_{<0}\setminus\{-e,-1,1-g\}\bigr)\cup\{0,f-1,1\}$.  Hence the Lie
algebra $\mathfrak g_t$ generated by $\{R_e(t)\}$ contains elements of bead-number $3$,
for infinitely many triples $(e,f,g)$.
\end{corollary}

\begin{proof}
Legality and distinctness: the removed sites $-e,-1,1-g$ are all negative (using
$g\ge e+2\ge4$) hence in $M=\Z_{<0}$, and are distinct because $e\ge2$, and
$1-g\le-3<-1$, and $1-g\ne-e$ since $g\ne e+1$; the added sites $0,f-1,1$ are all
$\ge0$ hence outside $M$, and distinct because $f\ge3$.  Removed and added sets are
disjoint by sign.

Interference: interval $1$ is $(-e,0)$, interval $2$ is $(-1,f-1)$, interval $3$ is
$(1-g,1)$.  Then $b_2=-1\in(-e,0)$ (as $e\ge2$) while $b_2+f=f-1\ge2\notin(-e,0)$, so
$\kappa_{21}=-1$.  Both $b_3=1-g\le-e$ and $b_3+g=1$ lie outside $(-e,0)$, so
$\kappa_{31}=0$; thus $\kappa_{21}+\kappa_{31}=-1\ne0$.  And $b_3=1-g\le-3\notin(-1,f-1)$
while $b_3+g=1\in(-1,f-1)$ (as $f\ge3$), so $\kappa_{32}=1\ne0$.  The statistics are
$P_1=\#(M\cap(-e,0))=e-1$, $P_2=\#(M\cap(-1,f-1))=0$, $P_3=\#(M\cap(1-g,1))=g-2$, so
$P=e+g-3$ and Proposition~\ref{prop:three} gives
$t^{e+g-3}(t-t^{-1})(t^{-1}-t)=-t^{e+g-5}(t^{2}-1)^{2}$.

Uniqueness of the assignment.  Write $R=\{-e,-1,1-g\}$ and $A=\{0,f-1,1\}$ and consider
any bijection $R\to A$ whose displacement multiset is $\{e,f,g\}$.  The nine possible
displacements are: from $-e$, $\;e$, $e+f-1$, $e+1$; from $-1$, $\;1$, $f$, $2$; from
$1-g$, $\;g-1$, $f+g-2$, $g$.  A displacement equal to $1$ is impossible ($e,f,g\ge2$),
which rules out $-1\mapsto0$.  Suppose $-e\mapsto0$; then $-1\mapsto1$ and
$1-g\mapsto f-1$, with displacements $\{e,2,f+g-2\}$, forcing $2\in\{f,g\}$ --- but
$f\ge3$ and $g\ge4$.  Suppose $-e\mapsto f-1$ (displacement $e+f-1$); since $e+f-1>e$ and
$e+f-1>f$, we need $e+f-1=g$, and then $-1\mapsto1$, $1-g\mapsto0$ give displacements
$\{g,2,g-1\}=\{e,f,g\}$, i.e.\ $\{2,g-1\}=\{e,f\}$; with $e\ge2\le f$ and $f\ge3$ this
forces $e=2$, $f=g-1$, the excluded case.  Suppose $-e\mapsto1$ (displacement $e+1$);
then $-1\mapsto f-1$ and $1-g\mapsto0$ give $\{e+1,f,g-1\}=\{e,f,g\}$, i.e.\ $e+1=g$ and
$g-1=e$, contradicting $g\ge e+2$.  So the intended assignment is the only one.
\end{proof}

\begin{remark}
The excluded case is not an artefact: for $(e,f,g)=(2,f,f+1)$ there really are two
assignments, and they happen to contribute equally, doubling the coefficient rather than
cancelling it.  Engine~A confirms $-2t^{5}+4t^{3}-2t$ on $s_{(3,3,3)}$ for $(2,3,4)$,
exactly twice the single-assignment value.  Verified: $31/31$ triples in the range
$2\le e\le4$, $3\le f\le6$, $e+2\le g\le8$ satisfying the hypotheses, and $92/92$
three-bead matrix elements against the full sum of Proposition~\ref{prop:three}.
\end{remark}
...
\begin{enumerate}
\item \textbf{Gaps 1 and 2 of the first draft are closed} by
Proposition~\ref{prop:psiwitness} and Corollary~\ref{cor:three}, in \S\ref{sec:closed}.
I record what the obstruction actually was, because I misjudged it: I had written
``I have not proved that no cancellation occurs'', but Theorem~\ref{thm:matrix}(2)
already says the two-bead matrix element at a given target is a \emph{single} term when
$e\ne f$ --- the pair $(b,c)$ is recovered from $M$ and $M'$.  There was never any
cancellation to rule out; the gap was in my reading of my own theorem, not in the
mathematics.  The same collapse happened one level up: the three-bead sector factorises
(Proposition~\ref{prop:three}) because the four orderings pair off, and only a finite
check of assignment-uniqueness remained.

\item \textbf{The Lie algebra $\mathfrak g_t$ is not identified --- and it is \emph{not}
$\mathcal W_{1+\infty}$.}  I have shown what $\mathfrak g_t$ is not
(Corollaries~\ref{cor:noclose}--\ref{cor:unbdd}, \ref{cor:three}).  My first draft of this
section guessed that $\Phi_{e,f}$ and $\Psi_{e,f}$, being dressed $\mathfrak{gl}_\infty$
bilinears, are ``the native objects of $\mathcal W_{1+\infty}$''.  \textbf{That guess is
wrong, and my own browse log of this morning says why}: $\mathcal W_{1+\infty}$ consists of
the $\sum_kf(D)z^{k}$ with $f$ \emph{polynomial}, whereas a single occupation projector
$n_j=\psi_j\psi_j^{*}$ is a delta in that variable, not a polynomial symbol.  So
$n_j\in\mathfrak{gl}_\infty\setminus\mathcal W_{1+\infty}$, and every dressing appearing in
Theorem~\ref{thm:main} is built from exactly such projectors.  The correct statement of the
exposure is therefore narrower than I first wrote, and correspondingly more useful:
$\Phi_{e,f}$ and $\Psi_{e,f}$ lie in the fermionic-degree filtration of
$\mathfrak{gl}_\infty$ (degree $\le2(e{+}f)$ and $\le4$ respectively, before dressing), not
in $\mathcal W_{1+\infty}$; what is unchecked is whether \emph{that} filtration's
cross-rank commutators have a name.  I still have not searched Bloch--Okounkov.  This
session had no network.  \textbf{Nothing in this note claims novelty.}

\item \textbf{Identifier collision, recorded not silently fixed.}  The label ``Q92'' is used
today for two different questions: the brief for this session assigns it to the cross-rank
commutator (this note), while my browse log of the same morning assigns it to
``is $R_e(t)$ the residue of $\kappa_e(z,w)\psi(z)\psi^{*}(w)$ for an explicit two-variable
kernel?''.  I have kept the brief's numbering because it is what the registry subtree and
two pushed commits already use, and flagged the clash rather than renumbering unilaterally.
The two questions are related, and in a way that is evidence for the other one:
that log observes ``my two-body/hole-counting result is exactly the condition for
$\kappa_e$ to be genuinely two-variable''.  Theorem~\ref{thm:main} supplies a sharp form of
that condition --- the two-body part $\Psi_{e,f}$ is nonzero exactly when $e,f\ge2$
(Corollary~\ref{cor:e1}, Proposition~\ref{prop:psiwitness}) --- so any kernel $\kappa_e$
must be genuinely two-variable for $e\ge2$ and may be taken one-variable at $e=1$.
