...
\subsection{A rigidity on the way: the one-parameter deformation factors when $p=n$}

Before homogenizing, I deformed off the Schur point in the natural one-parameter direction:
keep $a_1=a_2=c_1=1$, $b_1=x_i$, turn on $b_2=\varepsilon$, and let the free-fermion
condition determine $c_2=1+\varepsilon x_i$. This stays free-fermion for all $\varepsilon$.

\begin{observation}\label{obs:factor}
With $p$ particles in $n$ rows and $\mathrm{left}=\mathrm{right}=0$, if $p=n$ then
\[
Z_\varepsilon \;=\; Z_0\cdot\prod_{i=1}^{n}(1+\varepsilon x_i)^{\,n},
\]
verified exactly for $n=2$ with $m=4,5,6$ and $n=3$ with $m=6,7$, over every admissible
boundary. The factor is independent of the boundary condition and of $m$. If $p<n$ the
factorization \textbf{fails}: for $n=3$, $p=2$ no exponent $k\le 2m$ works for any of the
three boundaries.
\end{observation}

So along this deformation the model is rigid exactly on the diagonal $p=n$ --- the
deformation is a gauge factor there and produces nothing new --- and genuinely deforms
only when $p<n$. That is what sent me to the homogeneous setting, where $\N(\cdot)$ is
actually defined. Scripts \texttt{deform.py}, \texttt{factortest.py}, \texttt{gf1.py},
\texttt{gf2.py}, \texttt{general\_ff.py}.
...
\texttt{deform.py}, \texttt{factortest.py}, \texttt{gf1.py}, \texttt{gf2.py},
\texttt{general\_ff.py} (Observation~\ref{obs:factor}).
