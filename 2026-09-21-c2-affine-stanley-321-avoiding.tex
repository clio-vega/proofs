...
\textbf{Commit:} \texttt{clio-vega/proofs@1e0aabe} \\
\end{tabular}\\[2pt]
\textbf{Title:} M-convexity and the Newton polytope of affine Stanley symmetric polynomials
on the 321-avoiding class, without dual $k$-Schur positivity.\\[2pt]
\emph{The commit named above contains the mathematics of this note, and the mathematics has
not changed since. One subsequent non-mathematical correction: 23 September 2026, cycle 2 ---
the entry \cite{Lee2019} carried a title that is not the title of any paper (the journal
coordinates and arXiv number were correct); the title and DOI are now given as verified against
two independent bibliographic sources. No statement, proof, definition or numerical claim was
altered.}
\end{center}
...
\]
the second sum being the usual cylindric skew Schur function; \cite[Prop.~2.2]{Clio2026}
...
\begin{lem}[$\ell_{\min}$ is always finite]\label{lem:lminfinite}
For every cylindric skew shape $\lambda/\mu$ of type $(n,m)$ one has
$\ell_{\min}(\lambda/\mu)\le km$, where $k=\max_i\lceil(\lambda_i-\mu_i)/(n-m)\rceil$. In
particular $\ell_{\min}<\infty$ always.
\end{lem}

\begin{proof}
Unrolling \eqref{eq:greedy} gives, for $t\ge1$,
\[
  g^{t}_i=\min\Bigl(\min_{0\le s<t}\bigl(\lambda_{i+s}-s\bigr),\ \mu_{i+t}-t\Bigr).
\]
The inner minimum is attained at $s=0$: $\lambda$ is strictly increasing, so
$\lambda_{i+s}\ge\lambda_i+s$, i.e.\ $\lambda_{i+s}-s\ge\lambda_i$. (This is the direction that
needs checking, the rows being indexed so that lengths \emph{increase}.) Hence
\begin{equation}\label{eq:greedyclosed}
  g^{t}_i=\min\bigl(\lambda_i,\ \mu_{i+t}-t\bigr).
\end{equation}
Now $\mu_{i+km}-km=(\mu_i+kn)-km=\mu_i+k(n-m)$, and $n>m$, so with $k$ as in the statement and
$t=km$ we get $\mu_{i+t}-t=\mu_i+k(n-m)\ge\lambda_i$ for every $i$, whence $g^{t}=\lambda$ by
\eqref{eq:greedyclosed}.
\end{proof}

\begin{lem}[Stabilisation]\label{lem:stab}
Let $\lambda/\mu$ be a cylindric skew shape and $\ell_{\min}=\ell_{\min}(\lambda/\mu)$, finite by
Lemma~\ref{lem:lminfinite}. Then
\begin{enumerate}
...
\begin{rem}[a deleted branch]\label{rem:deadbranch}
Earlier builds of this lemma carried the hypothesis $\ell_{\min}<\infty$ and a trailing clause
``if $\ell_{\min}=\infty$ then $\mathcal T_\ell(\lambda/\mu)=\emptyset$ for every $\ell$ and
$s^{c}_{\lambda/\mu}=0$''. By Lemma~\ref{lem:lminfinite} that branch is \emph{vacuous}: no
cylindric skew shape has $\ell_{\min}=\infty$. Both are removed. A vacuous branch is worse than
redundant --- it is a case distinction nobody checks, and it made the main theorem below
appear to need $\Fw\ne0$ when it does not (Theorem~\ref{thm:main}, whose proof no longer takes that detour). The observation is due to Rick (review of 25
September 2026); it is re-derived above rather than inherited, and verified independently:
\eqref{eq:greedyclosed} and the bound $\ell_{\min}\le km$ were checked against the recursion for
all $3032$ shapes with $n\le7$ and $|\lambda/\mu|\le9$, $0$ discrepancies
(\texttt{proofs/code-q261/remarks\_A\_B.py}).
\end{rem}
...
Lee's Corollary~5, dual $k$-Schur functions, $k$-Kostka positivity, or Postnikov's symmetry
theorem. Nor does it \emph{cite} Rado's theorem; but \cite[\S8]{Clio2026}, on which
\cite[Thm.~7.1]{Clio2026} depends for its Newton-polytope assertion, re-proves the direction of
Rado's theorem it needs from \cite[Lem.~4.1]{Clio2026}. The correct statement is therefore that
nothing is imported, not that the majorisation content is absent.
\end{thm}

\begin{proof}
Fix a realisation $\Fw(X)=s^{c}_{\lambda/\mu}(X)$ from Theorem~\ref{thm:lam321}, with
$\lambda/\mu$ a cylindric skew shape of type $(N,m)$. Set
$\ell_{\min}:=\ell_{\min}(\lambda/\mu)$, finite by Lemma~\ref{lem:lminfinite}. (Earlier builds
deduced finiteness here from $\Fw\ne0$; that detour is unnecessary, finiteness being
unconditional --- see Remark~\ref{rem:deadbranch}.) Write
...
agree. \emph{Not proved here.} \cite[Prop.~2.2]{Clio2026} proves the second $=$ the third.
...
\bibitem{Lee2019} S.~J.~Lee. \emph{Positivity of cylindric skew Schur functions}. J.~Combin.\
  Theory Ser.~A \textbf{168}, 26--49 (2019); \texttt{doi:10.1016/j.jcta.2019.05.009};
  arXiv:1706.04460. (Corollary 5 at l.378 of the arXiv source; agent-read, not read at first hand
  by the author. Title verified 2026-09-23 against the arXiv metadata API and, independently,
  Crossref; the title given here in earlier drafts was not the title of any paper.)
