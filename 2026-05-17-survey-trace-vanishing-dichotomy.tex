...
outright), and $68$ are descent-$0$ SYTs with pair $(\ell, \ell+1)$
diagonal (D-class).  For these, $v_T$ has nonzero components in both
$\Eplus_\ell$ and $\Eminus_\ell$, and the seminormal projection no longer
forces $c_{T,T} = 0$ directly.  These ``interior cancellations'' are not
explained by any of the three diagnostics above; characterising them is
the natural next-horizon refinement of per-SYT analysis.
...
$\lambda = (3,3,1,1,1)$
(Section~\ref{sec:diag-open}) remain unexplained by the three diagnostics
collected here.
...
B.~Brubaker, V.~Buciumas, D.~Bump, and H.~P.~A.~Gustafsson.
\emph{Demazure--Lusztig operators and metaplectic Whittaker functions.}
\texttt{arXiv:2410.07960}, 2024.

\bibitem{gwzj2025}
S.~Gunna, M.~Wheeler, and P.~Zinn-Justin.
\emph{$K$-theoretic Schubert calculus via Demazure--Lusztig operators.}
\texttt{arXiv:2504.19205}, 2025.

\bibitem{cheng2025}
W.~Cheng et al.
\emph{Equivariant $K$-theory and Hecke-algebra evaluations on flag varieties.}
