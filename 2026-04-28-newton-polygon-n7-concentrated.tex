...
\item \textbf{Splitting field.}  Theorems T2 and T3 give the multiset of valuations
of the roots, but do not address whether $g$ splits over $\Q(q)$ or only over a
finite extension.  In both cases the Newton polygon is ``regular'' at the slope
$-3$ (length $1$): so by Newton-polygon factorisation
\cite[Ch.~II,~Cor.~6.5]{Neukirch}, $g$ factors as $g = g_1 \cdot g_0$ in
$\Q(q)[t]$ where $g_1$ is monic of degree $1$ (the atom$^3$-carrying root, defined
over $\Q(q)$) and $g_0$ has all roots of $\atom$-valuation $0$.  In particular,
\emph{the atom$^3$ eigenvalue is rational over $\Q(q)$}.  The remaining roots
(those of $g_0$) need not be: at $V_{(5,2)/n=7}$, $g_0$ has degree $4$ and is the
quartic carrying the leading-$2$ surprise.
