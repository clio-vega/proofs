...
\emph{Closed form.} The recursion admits a clean closed-form solution
via a bijection between saturated descending chains and standard Young
tableaux of the \emph{decoration shape}
...
\begin{theorem}[Closed-form SYT formula]\label{thm:syt}
Subject to Conjecture~\ref{conj:main} (equivalently, to
Theorem~\ref{thm:main} modulo its technical ingredients),
...
\toprule
$\lambda$ & $r$ & $\lambda^{(\ge 2)}$ & $f^{\lambda^{(\ge 2)}}$ & $\dim$ & match\\
\midrule
Hook $(k, 1^*)$ & $1$ & $(k - 1)$ & $1$ & $1$ & \checkmark\\
$(2^a, 1^*)$ & $1$ & $(1^a)$ & $1$ & $1$ & \checkmark\\
$(3, 2, 1^*)$ & $2$ & $(2, 1)$ & $2$ & $2$ & \checkmark\\
$(4, 2, 1^*)$ & $2$ & $(3, 1)$ & $3$ & $3$ & \checkmark\\
$(3, 3, 1^*)$ & $1$ & $(2, 2)$ & $2$ & $2$ & \checkmark\\
$(4, 3, 1^*)$ & $2$ & $(3, 2)$ & $5$ & $5$ & \checkmark\\
$(5, 2, 1^*)$ & $2$ & $(4, 1)$ & $4$ & $4$ & \checkmark\\
$(3, 2, 2, 1^*)$ & $2$ & $(2, 1, 1)$ & $3$ & $3$ & \checkmark\\
$(4, 2, 2, 1^*)$ & $2$ & $(3, 1, 1)$ & $6$ & $6$ & \checkmark\\
$(3, 3, 2, 1^*)$ & $2$ & $(2, 2, 1)$ & $5$ & $5$ & \checkmark\\
$(4, 3, 2, 1^*)$ & $3$ & $(3, 2, 1)$ & $16$ & (predicted) & --\\
\bottomrule
\end{tabular}
\end{center}

\noindent The closed form $f^{\lambda^{(\ge 2)}}$ matches the recursive
prediction in every tested case. The $r = 3$ row gives the closed-form
prediction $f^{(3, 2, 1)} = 16$ for $\lambda = (4, 3, 2, 1^*)$, refining
the open question at the end of the paper.
...
\begin{tabular}{lccccc}
\toprule
$\lambda$ & $n$ tested & $r$ & $\dim$ & refined prediction & $f^{\lambda^{(\ge 2)}}$\\
\midrule
Hook $(k, 1^{n-k})$, $k \ge 2$ & various $n$ & $1$ & $1$ & $1$ & $1$\\
$(2^a, 1^{n-2a})$, $a \ge 1$ & $n \ge 3a$ & $1$ & $1$ & $1$ & $1$\\
$(3, 2, 1^{n-5})$ & $8, 9$ & $2$ & $2$ & $2 = 1 + 1$ & $2$\\
$(4, 2, 1^{n-6})$ & $10, 11$ & $2$ & $3$ & $3 = 2 + 1$ & $3$\\
$(3, 3, 1^{n-6})$ & $10, 11$ & $1$ & $2$ & $2$ (single term) & $2$\\
$(4, 3, 1^{n-7})$ & $10, 11$ & $2$ & $5$ & $5 = 2 + 3$ & $5$\\
$(5, 2, 1^{n-7})$ & $10, 11$ & $2$ & $4$ & $4 = 3 + 1$ & $4$\\
$(3, 2, 2, 1^{n-7})$ & $10, 11$ & $2$ & $3$ & $3 = 1 + 2$ & $3$\\
$(4, 2, 2, 1^{n-8})$ & $10, 11$ & $2$ & $6$ & $6 = 3 + 3$ & $6$\\
$(3, 3, 2, 1^{n-8})$ & $11$ & $2$ & $5$ & $5 = 3 + 2$ & $5$\\
\bottomrule
\end{tabular}
\end{center}

Every tested case agrees with the refined formula \emph{and} with the
closed-form SYT count $f^{\lambda^{(\ge 2)}}$ (Theorem~\ref{thm:syt}).
...
\paragraph{Closed form: solved.}
Section~\ref{sec:syt} resolves the closed-form question affirmatively.
The dimension equals the SYT count of the decoration shape,
\[
...
combinatorial statistic — the SYT count of the decoration shape — and
the chain length is $|\lambda^{(\ge 2)}| = |\lambda| - \ell(\lambda)$,
which gives a clean local statistic: each decoration cell contributes
one chain step.

\paragraph{$r = 3$ prediction sharpened.}
The closed form gives $\dim B_{\ell + 1}^{(4, 3, 2, 1^*)} V_\lambda =
f^{(3, 2, 1)} = 16$, matching the recursive sum $5 + 6 + 5 = 16$ from
above. This is now a single hook-product computation rather than a
chain enumeration.

\paragraph{Status.} The formula \eqref{eq:main} matches the
computational data on every tested family ($r \in \{1, 2\}$,
$n \le 11$). The closed form \eqref{eq:syt} follows from \eqref{eq:main}
by the chain-SYT bijection of Section~\ref{sec:syt}. The general proof
of \eqref{eq:main} is structurally complete modulo ingredients~(i)--(ii)
of Section~\ref{sec:proof}. The next concrete target is the $r = 3$
verification at $n = 13$, where the closed form predicts
$\dim = f^{(3, 2, 1)} = 16$.
